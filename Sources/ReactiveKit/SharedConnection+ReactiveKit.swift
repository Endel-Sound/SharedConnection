//
//  SharedConnection+ReactiveKit.swift
//  Endel
//
...
public extension Signal where Error == Never {
    ...
    {
        self = AnyPublisher<Element, Error>.init(sharedConnectionService, sharedDataFor: key,
    ...
    init<Key: SharedStateKeyProtocol>(
        _ sharedConnectionService: SharedConnectionService = SharedConnectionManager.shared,
        sharedDataFor key: Key,
        policy: Set<SharedConnectionPolicy.Receive> = []) where Key.Value == Element
    {
...
public extension SignalProtocol where Error == Never {
    ...
    func receiveSharedData<Key: SharedStateKeyProtocol>(
        ...
        key: Key,
        policy: Set<SharedConnectionPolicy.Receive> = []
    ) -> Signal<Element, Error> where Key.Value == Element
    {
    ...
    {
        return sharedConnectionService.provideSharedData(key: key, signal: self.toPublisher(), origin: origin,
                                                         policy: policy, transform: transform)
            .toSignal()
    }
    ...
    func provideSharedData<Key: SharedStateKeyProtocol>(
        ...
        key: Key,
        origin: SharedConnectionPolicy.Provide.Origin,
        policy: Set<SharedConnectionPolicy.Provide> = []) -> Signal<Element, Error> where Element == Key.Value
    {
    ...
    func syncSharedData<Key: SharedStateKeyProtocol>(
        ...
        transform: @escaping (Element) -> Key.Value) -> SafeSignal<Key.Value>
    {
        return sharedConnectionService.syncSharedData(key: key, signal: self.toPublisher(), origin: origin,
    ...
    func syncSharedData<Key: SharedStateKeyProtocol>(
        ...
        origin: SharedConnectionPolicy.Provide.Origin,
        receivePolicy: Set<SharedConnectionPolicy.Receive> = [],
        providePolicy: Set<SharedConnectionPolicy.Provide> = []) -> SafeSignal<Key.Value> where Element == Key.Value
    {
